...
const carService = __importStar(require("../../src/services/carService"));
jest.mock("../../src/services/carService");
const mockedService = carService;
describe("Car Routes Integration", () => {
    it("GET /cars deve retornar 200 com lista de carros", async () => {
        ...
        const response = await (0, supertest_1.default)(app_1.default).get("/cars");
    ...
    it("GET /cars deve aceitar paginação via query page e limit", async () => {
        ...
        mockedService.getCars = jest.fn().mockResolvedValue(mockCars);
        const response = await (0, supertest_1.default)(app_1.default).get("/cars?page=2&limit=5");
        expect(response.status).toBe(200);
        expect(response.body).toEqual(mockCars);
    ...
    it("GET /cars/:id deve retornar 200 se carro existir", async () => {
        ...
        const response = await (0, supertest_1.default)(app_1.default).get("/cars/1");
        expect(response.status).toBe(200);
    ...
    it("GET /cars/:id deve retornar 404 se carro não existir", async () => {
        ...
        const response = await (0, supertest_1.default)(app_1.default).get("/cars/999");
        expect(response.status).toBe(404);
    });
});
//# sourceMappingURL=carRoutes.test.js.map